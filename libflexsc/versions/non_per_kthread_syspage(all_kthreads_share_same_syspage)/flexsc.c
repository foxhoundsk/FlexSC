 ...
 * PTHREAD_BROADCAST.
 * 
 * Note that in this implementation, if get_free_syscall_entry() is failed (which
 * means that syspage is fulled), we may made kernel-visible thread to call
 * flexsc_wait directly. And user threads waiting with spinlock.
 * 
...
static int init_lock_syspage(struct flexsc_init_info *info)
{
    ...
    error = mlock(info->sysentry, round_up(info->total_bytes, getpagesize()));
...
static int init_map_syspage(struct flexsc_init_info *info)
{
    ...
    size_t total = SYSPAGE_PER_TASK * (sizeof(struct flexsc_sysentry));
    ...
    for (int i = 0; i < SYSPAGE_PER_TASK; i++) {
        entry[i].rstatus = FLEXSC_STATUS_FREE;
    }
    /* debug use */ entry[1].sysnum = 12345;
...
long flexsc_exit(void)
{
    ...
    ret = syscall(401);
    puts("wait for resources being used to stop");
    sleep(3);
...
static void create_kv_thread(struct flexsc_init_info *info)
{
    ...
    kv_handle_tmp = (struct kv_handle_syspg_num*) aligned_alloc(pgsize, sizeof(struct kv_handle_syspg_num) * cpu_qnt); 
