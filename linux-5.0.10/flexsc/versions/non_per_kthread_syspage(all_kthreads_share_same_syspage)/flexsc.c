...
static int create_kthread(struct flexsc_init_info *info)
{
    ...
    for (idx = 0, kthread_list_tmp = &kthread_list;;) {
        ...
        kthread_list_tmp->kthread_ts = kthread_create_on_cpu(systhread_fn, &k_handle_syspg[idx], kthread_list_tmp->cpu, FLEXSC_KTHREAD_NAME".*%u");
...
static int create_workstruct(struct flexsc_init_info *info)
{
    ...
    flexsc_works = (struct work_struct*) kmalloc(sizeof(struct work_struct) * info->npages, GFP_KERNEL);
    ...
    for (i = 0; i < info->npages; i++) {
         ...
         */
        FLEXSC_INIT_WORK(&flexsc_works[i], flexsc_wq_worker, &(info->sysentry[i]), &(sys_regs[i]));
    }
...
long do_flexsc_register(struct flexsc_init_info __user *info)
{
    ...
    pages = 1; /* if we want to map pages more than one page, we need to call kmap iteratively since it map one page at each call */
    ...
    k_info->sysentry = (struct flexsc_sysentry*) kmap(k_sysent_pg);
...
static int systhread_fn(void *args)
{
    struct k_handle_syspg_num *syspg_num = (struct k_handle_syspg_num*) args;
    int i;
    ...
    while (!kthread_should_stop()) {
        ...
        for (i = 0; i < k_info->npages; i++) {
            ...
            if (FLEXSC_STATUS_MARKED == k_info->sysentry[i].rstatus) {
                k_info->sysentry[i].rstatus = FLEXSC_STATUS_BUSY;
                
                /* according to the doc of CMWQ, we guarantee that the work is queued to specific cpu in most case */
                //queue_work(flexsc_workqueue, &flexsc_works[i]);
                cpu = (cpu % 7) + 1; /* 1~7 */
                queue_work_on(cpu, flexsc_workqueue, &flexsc_works[i]);
            }
        }
